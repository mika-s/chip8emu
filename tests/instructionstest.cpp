...
void InstructionTest::given_3XNN_andVXisNNgotoNextPC() {
    ...
    emulator.nextInstruction();
    QCOMPARE(emulator.PC(), 0x206);     // PC += 2 ordinary, PC += 2 because if(V[2] == 0x55) is true
}
...
void InstructionTest::given_4XNN_andVXisNotNNgotoNextPC() {
    Emu::Emulator emulator = Emu::Emulator(timer, settings);
    emulator.loadFile({
                              0x62, 0x55,           // V[2] = 0x55
                              0x42, 0x56            // if(V[2] != 0x56) --> gotoNextPC()
                      });
    ...
    emulator.nextInstruction();
    QCOMPARE(emulator.PC(), 0x206);     // PC += 2 ordinary, PC += 2 because if(V[2] != 0x56) is true
}

void InstructionTest::given_4XNN_andVXisNNdoNotgotoNextPC() {
    Emu::Emulator emulator = Emu::Emulator(timer, settings);
    emulator.loadFile({ // if(Vx != NN)
                              0x62, 0x55,           // V[2] = 0x55
                              0x42, 0x55            // if(V[2] != 0x55) --> gotoNextPC()
                      });
